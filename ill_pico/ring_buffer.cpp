...
void ring_buffer::print_buff(bool sel, int node) {

    if (sel) {
        for (int j = 0 ; j < BUFFER ; j++) {
            data_reads* data_print = get(tail_i +j);
            //Serial Print
            Serial.print(" Measured lux :"); Serial.print(data_print->out);
            Serial.print(" at node "); ; Serial.println(node);
            Serial.print(" At time : "); Serial.println(data_print->time);
    }
}
    else {
        for (int j = 0 ; j < BUFFER ; j++) {
            data_reads* data_print = get(tail_i +j);
            //Serial Print
            Serial.print(" PWM control % :"); Serial.print((data_print->u*100)/DAC_RANGE);
            Serial.print(" at node "); ; Serial.println(node);
            Serial.print(" At time : "); Serial.println(data_print->time);
    }
}
