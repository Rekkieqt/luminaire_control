...
pid::pid(float h_, float b_h_, float b_l_, float c_, float K_h_, float K_l_, float Ti_, float Tt_, float Td_, int N_)
...
void pid::set_reference(float r_){
  curr_r = r_;
  if (occ_st == OCC) { 
    // b_h = (curr_r - d)/(K_h*curr_r*G);
    if(feedback) I += curr_K*(curr_b*r_l - y_old) - K_h*(b_h*curr_r - y_old);
    curr_K = K_h;
    curr_b = b_h;
    r_h = curr_r;
  }
  else if (occ_st == UNOCC) {
    // b_l = (curr_r - d)/(K_l*curr_r*G);
    if(feedback) I += curr_K*(curr_b*r_h - y_old) - K_l*(b_l*curr_r - y_old);
    curr_K = K_l; 
    curr_b = b_l; 
    r_l = curr_r;
  }
...
float pid::get_disturbance(float u){
  return (u < 1 && u > 0) ? curr_r - curr_K*(curr_b-1)*curr_r - I : curr_r + u*(Ti/(curr_K*Tt) - G) - Ti/(curr_K*Tt)*v;
}

void pid::set_system_gain_n_dist(float G_, float d_){
  G = G_;
  d = d_;
}
...
float pid::computePWM(float y, float r){
  ...
  D = ad*D - bd*(y - y_old);
  v = feedback ? (P + I + D)/G : curr_K*(curr_b-1)*curr_r/G;
